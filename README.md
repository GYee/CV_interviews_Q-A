...
## 食用方法
请下载安装**Typora**，Typora 是一款支持实时预览的 Markdown 文本编辑器。它有 OS X、Windows、Linux 三个平台的版本，完全免费，体验非常好，推荐你使用。
...
**最后需要声明的一点是**：本文是作为个人学习总结的，我们不生产知识，只是知识的搬运工，主要做的工作是知识整合与逻辑梳理，参考了很多大神的博客及文章资料(文章中会注明参考的资料，感谢这些作者的贡献)，当然也会有自己的思考，不喜勿喷，如有错误，恳请指正！
...
|  2   |          |                过拟合和欠拟合的表现及解决方法                |
|  3   |          |                       代码实现卷及操作                       |
|  4   |          | BN层的深入理解及代码实现　(补充先加BN还是激活函数的区别、反向传播与求导) <br />另外还要着重分析下GN与BN相比的优势 |
|  5   |          |         NMS的工作机制及代码实现(要继续补充soft NMS)          |
|  6   |          |                      三种常见的激活函数                      |
|  7   |          |              ReLU函数在0处不可导，为什么还能用               |
|  8   |          |             Pooling层的作用以及如何进行反向传播              |
|  9   |          |               为什么输入网络前要对图像做归一化               |
|  10  |          | 理清深度学习优化函数发展脉络(添加随机梯度下降和梯度下降的区别) |
...
|  2   |          |     说说 incluede 后面使用双引号 " " 和尖括号 <> 的区别      |
|  3   |          |                    说一下 C++ 和 C 的区别                    |
|  4   |          |                    请问 C++ 有哪些新特性                     |
...
|  11  |          |        new / delete与 malloc / free 之间的关系与区别         |
|  12  |          |                  delete 和 delete[] 的区别                   |
|  13  |          |           说说 ++i 与 i++的区别及各自是怎么实现的            |
|  14  |          | 说一说你所理解的虚函数和多态及虚函数表具体是怎样实现运行时多态的 |
|  15  |          |                        纯虚函数的作用                        |
|  16  |          | 构造函数为什么一般不定义为虚函数？而析构函数一般写成虚函数的原因 |
|  17  |          |              子类析构时，会调用父类的析构函数吗              |
|  18  |          |               请你说一下静态函数和虚函数的区别               |
...
|  29  |          |               说说 C 语言是怎么进行函数调用的                |
|  30  |          |    说说 C 语言函数中参数的压栈顺序（顺便了解下变长参数）     |
|  31  |          |           说一下什么是右值引用，跟左值又有什么区别           |
|  32  |          |                请说说 C++ 的内存管理是怎样的                 |
|  33  |          |       C++ / C 的内存分配，详细说一下栈、堆、静态存储区       |
|  34  |          |                     C 语言的函数调用过程                     |
|  35  |          |             什么是内存泄露，有几种情况，怎么判断             |
|  36  |          |                 请你说一说 strcpy 和 strlen                  |
|  37  |          |                       定义和声明的区别                       |
...
|  68  |          |             STL 中的 vector 的实现，是怎么扩容的             |
|  69  |          |        说一下 STL 中迭代器的作用，与指针又有什么区别         |
|  70  |          |                  说一说 STL 迭代器删除元素                   |
|  71  |          |                      STL 有哪些基本组成                      |
|  72  |          |               STL 中 resize 和 reserve 的区别                |
|  73  |          |                     map[] 和 find 的区别                     |
